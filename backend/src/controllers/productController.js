...
exports.cleanupLastImages = async (req, res) => {
  try {
      ...
      if (images.length > 0) {
        ...
        if (img.public_id) {
          try {
            ...
            console.error("Cloudinary err:", e.message);
          }
        }

        await db.query(`DELETE FROM product_images WHERE id = ?`, [img.id]);
        deletedCount++;
        deletedDetails.push({ product_id: p.product_id, image_id: img.id, public_id: img.public_id });
      }
    }

    res.json({ success: true, deletedCount, deletedDetails });
  } catch (err) {
