...
const BASE_FOLDER = "C:\\Users\\Desgin\\Desktop\\temp";

async function run() {
  try {
    console.log("⏳ Starting TEMP folder image upload to ImageKit...");
    ...
    console.log(`📁 Found ${folders.length} product folders in temp.`);
    ...
    for (let i = 0; i < folders.length; i++) {
      ...
      console.log(`🔄 [${i+1}/${folders.length}] Product ${productCode}: Uploading ${files.length} images from temp...`);

      // 1. Delete ALL old images from DB for this product
      await db.query(`DELETE FROM product_images WHERE product_id = ?`, [productId]);

      // 2. Upload ALL local files in temp to ImageKit
